...
class CollectRender(pyblish.api.InstancePlugin):
    ...
    def process(self, instance):
        ...
        if not isinstance(layer, RenderLayerNode):
            self.log.debug("Skip collecting node, not a RenderLayer node")
            return
        ...
        with Gaffer.Context(layer.scriptNode().context()) as ctxt:
            ...
            for outplug in layer["outputs"].children():
                ...
                if not outplug["enabled"].getValue():
                    self.log.info(f"skipipng disabled output {aov_name}")
                    continue
        ...
        data = {
            ...
            "renderProducts": ARenderProduct(
                layer.scriptNode(), list(expected_files.keys())
            ),
